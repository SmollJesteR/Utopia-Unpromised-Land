...
class Entity():
    ...
    def update_health(self):
        ...
        elif self.target_health > self.max_hp:
            self.target_health = self.max_hp

    def update_energy(self):
        if self.target_energy < 0:
            self.target_energy = 0
        elif self.target_energy > self.max_energy:
            self.target_energy = self.max_energy


# Buat karakter dan musuh
BloodReaper = Entity(501, 500, 1000, 10, 3, "BloodReaper", scale=3)
...
while run:
    ...
    for character in [BloodReaper, DoomCultist, Medusa, Cyclops]:
        character.update()
        character.draw()
